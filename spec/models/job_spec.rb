...
describe Job do
  describe 'adding job to database' do
    it'should assign param values then save' do
      ...
      fake_values.should_receive(:desc=).with("desc")
      ...
      retval = Job.create_job({:title => "title", :desc => "desc", :reqs => "reqs", :desire => "desire", :link => "link", :city => "city", :state => "state", :companyname => "companyname"}, fake_user)
      retval == fake_values
    end
  end
  describe 'updating job in databas' do
    it'should update param values then save' do
      ...
      fake_values.should_receive(:valid?).and_return(true)
      fake_values.should_receive(:save!)
      retval = Job.update_attrbs({:title => "title", :desc => "desc", :reqs => "reqs", :desire => "desire", :link => "link", :city => "city", :state => "state", :companyname => "companyname"}, fake_values)
      retval == fake_values
    end
  end
end
